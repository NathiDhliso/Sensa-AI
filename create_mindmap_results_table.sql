-- SQL script to create the mindmap_results table for AWS Lambda backend
...
-- Insert initial job record function (optional)
CREATE OR REPLACE FUNCTION create_mindmap_job(
  p_job_id text,
  p_user_id uuid DEFAULT NULL,
  p_subject text DEFAULT 'Unknown Subject'
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  result_id uuid;
BEGIN
  INSERT INTO mindmap_results (job_id, user_id, subject, status)
  VALUES (p_job_id, p_user_id, p_subject, 'queued')
...
GRANT ALL ON TABLE mindmap_results TO authenticated;
GRANT EXECUTE ON FUNCTION create_mindmap_job TO authenticated;