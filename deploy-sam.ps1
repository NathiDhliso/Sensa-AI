#!/usr/bin/env pwsh
# Sensa AI Backend Deployment Script using AWS SAM
# This script builds and deploys the Sensa Mindmap backend infrastructure
...
function Test-Prerequisites {
    ...
    if (-not (Get-Command aws -ErrorAction SilentlyContinue)) {
        ...
        Write-Info "Please install AWS CLI: https://aws.amazon.com/cli/"
        exit 1
    }
    ...
    if (-not (Get-Command sam -ErrorAction SilentlyContinue)) {
        ...
        Write-Info "Please install SAM CLI: https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/install-sam-cli.html"
        exit 1
    }
    
    # Check Docker
    if (-not (Get-Command docker -ErrorAction SilentlyContinue)) {
        Write-Warning "Docker is not installed. SAM will use native builds (may have compatibility issues)"
    } else {
        ...
        try {
            ...
            Write-Warning "Docker is installed but not running. Starting Docker..."
...
function Invoke-SamBuild {
    ...
    Write-Info "Running: sam $($buildArgs -join ' ')"
...
function Start-LocalDevelopment {
    Write-Info "Starting local SAM development environment..."
    
    try {
        & sam local start-api --warm-containers EAGER --parameter-overrides "SupabaseServiceRoleKey=$([Environment]::GetEnvironmentVariable('SUPABASE_SERVICE_ROLE_KEY'))" "GoogleAIApiKey=$([Environment]::GetEnvironmentVariable('GOOGLE_AI_API_KEY'))"
    } catch {
...
# Main execution
function Main {
    Write-Info "=== Sensa AI Backend Deployment using AWS SAM ==="
    ...
    if ($WatchMode) {
        Write-Info "Starting in watch mode for local development..."
